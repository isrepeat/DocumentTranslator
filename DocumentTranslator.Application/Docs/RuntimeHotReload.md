...
Разметка меню находится в `DocumentTranslator.UI/Controls/AlarmActionsMenu.xaml`. Команды его содержимого получают привязки к ViewModel страницы через `Create` и runtime-фабрику контрола.
