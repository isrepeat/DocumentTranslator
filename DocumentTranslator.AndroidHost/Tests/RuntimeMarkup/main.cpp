...
#include "DocumentTranslator.UI/Control/AlarmList.h"
...
namespace mobileclock::tests::_details {
    ...
    void CheckAlarmForm(const std::string& project) {
        ...
        const auto scrollableListPath = project + "/DocumentTranslator.UI/Control/AlarmMelodyList.xaml";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        for (const std::string name : {"MainPage", "SettingsPage", "AddAlarmPage"}) {
            const auto path = project + "/DocumentTranslator.Application/UI/Page/" + name + ".xaml";
            const auto markup = Read(path);
            Check(session.preview_ReloadMarkup(name, markup, path, diagnostics), diagnostics);
        }
        const auto mainPath = project + "/DocumentTranslator.Application/UI/Page/MainPage.xaml";
        ...
        Check(session.preview_ReloadMarkup("MainPage", mainMarkup, mainPath, diagnostics), diagnostics);
        const auto templatePath = project + "/DocumentTranslator.UI/Control/AlarmList.xaml";
        ...
        const auto tabsPath = project + "/DocumentTranslator.UI/Control/TimelineTabs.xaml";
